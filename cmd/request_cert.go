...
func NewCertRequestCommand() cli.Command {
	return cli.Command{
		...
		Flags: []cli.Flag{
			...
			cli.StringFlag{
				Name:  "email",
				Usage: "Sets Email field of the certificate",
			},
			cli.StringFlag{
				Name:  "ip",
...
func newCertAction(c *cli.Context) {
	...
	csr, err := pkix.CreateCertificateSigningRequest(key, c.String("organizational-unit"), ips, domains, uris, c.String("organization"), c.String("country"), c.String("province"), c.String("locality"), c.String("email"), name)
	if err != nil {
