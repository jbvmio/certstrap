...
func CreateCertificateSigningRequest(key *Key, organizationalUnit string, ipList []net.IP, domainList []string, uriList []*url.URL, organization string, country string, province string, locality string, email string, commonName string) (*CertificateSigningRequest, error) {
	...
	if email != "" {
		...
		csrPkixName.ExtraNames = []pkix.AttributeTypeAndValue{
			{
